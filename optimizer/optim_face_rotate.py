...
class optimizer(base_optimizer):
    ...
    def set_input(self, input):
        ...
        self.landmark_trans = util.toVariable(255*self.landmark_trans).cuda()
        self.landmark_same = util.toVariable(255*self.landmark_same).cuda()
    ...
    def optimize_parameters(self, global_step):
        ...
        _, _, self.feat_landmark_trans = self.encoder(self.landmark_trans.detach())
        _, _, self.feat_landmark_same = self.encoder(self.landmark_same.detach())
        # print(self.feat_landmark_same.shape)

        ''' define output from decoder '''
        # output (0-1)
        self.output_trans = self.decoder(self.feat_identity.detach(), self.feat_landmark_trans.detach())
        self.output_same = self.decoder(self.feat_identity.detach(), self.feat_landmark_same.detach())
        # print(self.output_same.shape)
    ...
    def save_samples(self, test_dataloader, global_step=0):
        ...
        for i, data in enumerate(test_dataloader):
            ...
            _, _, feat_identity = self.encoder(image)
            _, _, feat_landmark_trans = self.encoder(landmark_trans)
            _, _, feat_landmark_same = self.encoder(landmark_same)
            # output normalized around mean / std
            output_trans = self.decoder(feat_identity, feat_landmark_trans)
            output_same = self.decoder(feat_identity, feat_landmark_same)
