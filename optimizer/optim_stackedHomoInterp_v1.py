...
class optimizer(base_optimizer):
    ...
    def _get_aux_nets(self):
        ...
        self.KGTransforms = nn.ModuleList([nn.DataParallel(nn.Conv2d(512, 512, 1)).cuda(), nn.DataParallel(nn.Conv2d(512, 512, 1)).cuda()])
    ...
    def _define_optim(self):
        ...
        self.optim_KGTransforms = torch.optim.Adam(self.KGTransforms.parameters(), lr=1e-4)
    ...
    def load(self, label='latest'):
        ...
        self._check_and_load(self.KGTransforms, save_dir.format('KGTransform', label))
    ...
    def stacked_encoder(self, image, k):
        ...
        for encoder in self.encoders[0:k+1]:
            f = encoder(enc_feats[-1].detach()) #detach to avoid interference between stacks 
            enc_feats.append(f)
    ...
    def compute_dec_loss(self):
        ...
        for i, feat in enumerate(self.feat):
            ...
            if i == 0:
               im_out = self.decoders[i](feat)
            else:
               rev_fea, im_out = self.decoders[i](feat)
            ...
            '''
            if i >= 1:
               self.loss[f'dec_rev_stack{i}'] = nn.MSELoss()(rev_fea, self.feat[i-1].detach()) 
    ...
    def compute_KGTransform_loss(self):
        ...
        #feat_T2 = self.KGTransform(self.feat[0])
        #self.loss['KGTransform'] = nn.MSELoss()(feat_T2, feat_T2)
        for i, feat in enumerate(self.feat):
            if i == 0:
               feat_T2 = self.KGTransforms[i](feat)
            else:
               feat_T2 = self.KGTransforms[i](self.decoders[i].module.reverse(feat))
    ...
    def compute_enc_int_loss(self):
        ...
        self.loss['enc_int'] += self.loss['KGTransform']
        '''
        ...
        for i, (interp_net, fea1, fea2) in enumerate(zip(self.interp_nets, stacked_feat1, stacked_feat2)):
            ...
            for attr_idx in range(n_branches):
                ...
                for strength in [0, 0.5, 1]:
                    ...
                    if i == 0:
                       out_tmp = self.decoders[i](interp_feat)
                    else:
                       _, out_tmp = self.decoders[i](interp_feat)
    ...
    def stacked_save_samples(self, global_step=0):
        ...
        for k, interp_net in enumerate(self.interp_nets):
            ...
            if k == 0:
              out_now = self.decoders[k](feat.detach())
            else:
              _, out_now = self.decoders[k](feat.detach())
